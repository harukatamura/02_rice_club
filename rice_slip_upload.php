...
//==================================================================================================
	...
	$prgname = "精米倶楽部伝票データ取込";
	$prgmemo = "　精米倶楽部の佐川伝票の取込を行います。";
	...
	if($filepath<>""){
		...
		foreach ($file as $line) {
			...
			if($line[0] <> "お問い合せ送り状No."){
				$_update[$i] = "";
				$_update[$i]  .= " , slipnumber = ".sprintf("'%s'", $line[0]);
				$_update[$i]  .= " WHERE ship_idxnum = ".sprintf("'%s'", $line[1]);
				$_update[$i]  .= " AND slipnumber = ''";
				++$i;
			}
		}
		if($i > 1000){
...
<head>
	...
	<script type="text/javascript">
		...
		function Mclk_Upload(){
			//画面項目設定
			document.forms['frm'].action = './rice_slip_upload.php';
			...
			<div id="formWrap">
				...
				<form name="frm" method = "post" enctype="multipart/form-data" action="./barcode_upload_manual.php">
					...
					<?php if($i > 0){ ?>
						...
						※すでに伝票番号が登録されているデータはスキップして登録されます。</p>
					<?php } ?>
					</div>
				</form>
			</div>
